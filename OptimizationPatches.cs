...
namespace OstronautsPerfOpt
{
    ...
    // ========================================
    // VISUAL DEFERRAL: Transpiler on UpdateManual
    // ========================================

    [HarmonyPatch]
    public static class Patch_UpdateManual_VisualDeferral
    {
        static MethodBase TargetMethod()
        {
        ...
        {
            if (!PerfOptPlugin.CfgVisualDeferral)
                return instructions;

            var codes = new List<CodeInstruction>(instructions);
            var refreshAnimMethod = AccessTools.Method(typeof(CondOwner), "RefreshAnim");
            var registerMethod = AccessTools.Method(
                typeof(PerfOptPlugin), "RegisterForVisualUpdate",
                new[] { typeof(CondOwner) });

            if (registerMethod == null)
            {
                PerfOptPlugin.Log.LogWarning(
                    "[VIS-DEF] Could not find RegisterForVisualUpdate method. Deferral NOT applied.");
                return instructions;
            }
            ...
            {
                PerfOptPlugin.Log.LogWarning(
                    "[VIS-DEF] Transpiler: could not find RefreshAnim(). Deferral NOT applied.");
                return instructions;
            }

            for (int i = cutStart; i < codes.Count; i++)
            {
            ...
            {
                PerfOptPlugin.Log.LogWarning(
                    "[VIS-DEF] Transpiler: could not find Ret. Deferral NOT applied.");
                return instructions;
            }

            var newCodes = new List<CodeInstruction>(codes.Count + 4);
            for (int i = 0; i < cutStart; i++)
                newCodes.Add(codes[i]);

            newCodes.Add(new CodeInstruction(OpCodes.Ldarg_0));
            newCodes.Add(new CodeInstruction(OpCodes.Call, registerMethod));
            newCodes.Add(new CodeInstruction(OpCodes.Ret));

            PerfOptPlugin.Log.LogInfo(
                $"[VIS-DEF] Transpiler: cut {cutStart}-{cutEnd - 1}, " +
                $"replaced with RegisterForVisualUpdate(this)");
